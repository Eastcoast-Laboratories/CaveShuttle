...
# Paths
BUILD_GRADLE="caveshuttle/app/build.gradle"
...
if [ ! -f "$CHANGELOG_MD" ]; then
  ...
  if grep -q "## ${VERSION_NAME} " "$CHANGELOG_MD"; then
    # Version exists, update it instead of adding a new entry
    echo "Updating existing entry for ${VERSION_NAME} in CHANGELOG.md"
    
    # Get line number of the version header
    VERSION_LINE=$(grep -n "## ${VERSION_NAME} " "$CHANGELOG_MD" | cut -d ':' -f1)
    ...
    NEXT_VERSION_LINE=$(tail -n +$((VERSION_LINE + 1)) "$CHANGELOG_MD" | grep -n "^## " | head -n 1 | cut -d ':' -f1)
    ...
    # append rest of original file, but skip first line
    tail -n +2 "$CHANGELOG_MD" >> "$TEMP_FILE"
  fi
...
if [ ! -f "$CHANGELOG_DE_MD" ]; then
  ...
  # Check if the current version already exists in the changelog
  if grep -q "## ${VERSION_NAME} " "$CHANGELOG_DE_MD"; then
    # Version exists, update it instead of adding a new entry
    echo "Updating existing entry for ${VERSION_NAME} in CHANGELOG_de.md"
    
    # Get line number of the version header
    VERSION_LINE=$(grep -n "## ${VERSION_NAME} " "$CHANGELOG_DE_MD" | cut -d ':' -f1)
    ...
    # Find the next version header or end of file
    NEXT_VERSION_LINE=$(tail -n +$((VERSION_LINE + 1)) "$CHANGELOG_DE_MD" | grep -n "^## " | head -n 1 | cut -d ':' -f1)
    ...
    # append rest of original file, but skip first line
    tail -n +2 "$CHANGELOG_DE_MD" >> "$TEMP_FILE"
  fi
...
echo "Fastlane-Changelogs: de/${VERSION_CODE}.txt und en-US/${VERSION_CODE}.txt"
